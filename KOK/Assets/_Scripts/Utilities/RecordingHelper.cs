...
namespace KOK
{
    public class RecordingHelper : MonoBehaviour
    {
        ...
        {
            string zipFilePath = Application.persistentDataPath + "/AudioProcess/" + localAudioFilePath + ".zip";
            string extractDir = Application.persistentDataPath + "/AudioProcess/";
            ...
            FirebaseStorageManager.Instance
                .DownloadVoiceRecordingFile(
                    ...
                    () =>
                    {
                        wavFilePath = ExtractWavFileFromZip(zipFilePath,
                            extractDir);
        ...
        IEnumerator AssignAudio(Action<AudioClip> onSuccess)
        {
            ...
            {
                string fullPath = Application.persistentDataPath + "/AudioProcess/" + Path.GetFileNameWithoutExtension(wavFilePath) + ".wav";
                var audioClip = WavFileToAudioClip(fullPath);
                Debug.Log(fullPath + "\n" + audioClip);
        ...
        {
            Debug.Log("Extract to: " + destinationFolder);
            try
            {
        ...
        private void WriteWavFile(Stream stream, AudioClip clip)
        {
        ...
        private AudioClip WavFileToAudioClip(string wavFilePath)
        {
        ...
        private AudioClip WavBytesToAudioClip(byte[] fileBytes, int offsetSamples = 0, string name = "wav")
        {
