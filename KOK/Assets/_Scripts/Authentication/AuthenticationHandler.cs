...
namespace KOK
{
    public class AuthenticationHandler : MonoBehaviour
    {
        ...
        #endregion
        #region VerificationSuccessMessage
        private const string SS_Verify_Success = "Xác thực thành công, vui lòng đăng nhập lại!";
        #endregion
        ...
        [SerializeField] private TMP_Text verificationSuccessMessage;
        ...
        public void OnLoginButtonClick()
        {
            ...
            verificationSuccessMessage.text = "";
        ...
        public void OnVerificationSuccess(ResponseResult<Account> responseResult)
        {
            ...
            SwitchToLogin();
            verificationSuccessMessage.text = SS_Verify_Success;
        }
        ...
        private void CheckInactive()
        {
            ...
            {
                ApiHelper.Instance.GetComponent<AccountController>().GetAccountByIdCoroutine(Guid.Parse(PlayerPrefsHelper.GetString(PlayerPrefsHelper.Key_AccountId).Trim()), OnLoadAccountCheckSuccess, OnLoadAccountFailed);
            }
            else
            {
                SwitchToLogin();
        ...
        private void OnLoadAccountCheckSuccess(Account account)
        {
            if (account != null)
            {
                if (account.AccountStatus == AccountStatus.INACTIVE)
                {
                    // handle logic when account is inactive/banned
                    SetLoginErrorMessage("Tài khoản của bạn không được phép đăng nhập vào hệ thống!");
                    PlayerPrefsHelper.DeleteLoginData();
                }
                else if (account.AccountStatus == AccountStatus.NOT_VERIFY)
                {
                    VerificationEmail = string.Empty;
                    SendAndSwitchToVerification(new ResponseResult<Account>()
                    {
                ...
                else if (account.AccountStatus == AccountStatus.ACTIVE)
                {
                    PlayerPrefsHelper.SetAccountData(account);
                    SceneManager.LoadScene("v_home");
                }
            }
        }

        private void OnLoadAccountFailed(string value)
        {
            // 
            PlayerPrefsHelper.DeleteLoginData();
