...
public class PlayerNetworkBehavior : NetworkBehaviour, IComparable<PlayerNetworkBehavior>, IComparer<PlayerNetworkBehavior>
{
    ...
    IEnumerator AutoStopRecording()
    {
        ...
        //Debug.LogError(this.name + "AutoStopRecording");
        //StopRecording();
    }
    ...
    float createRecordingCooldown = 0;
    IEnumerator CreateRecording()
    {
        yield return new WaitForSeconds(0.1f);
        createRecordingCooldown += Time.deltaTime;
        Debug.LogError(createRecordingCooldown);
        ...
        if (PlayerRole == 0)
        {
            ...
            if (singerAccountIds.Count == 0 || singerAccountIds.Count != singerAudioUrls.Count || createRecordingCooldown <= 30)
            {
                Debug.LogError("Room recording is NOT ready: " + singerAccountIds.Count + " | " + (singerAccountIds.Count != singerAudioUrls.Count));
                StartCoroutine(CreateRecording());
                yield break;
            }
            else
            {
                Debug.LogError("Room recording READY: " + singerAccountIds.Count + " | " + (singerAccountIds.Count != singerAudioUrls.Count));
    ...
    public void StopRecording()
    {
        ...
        try
        {
            StopCoroutine(AutoStopRecording());
        }
        catch
        {
    ...
    public void PlayAnimation(AnimationName animation)
    {
        ...
        //Debug.LogError(characterAnim.GetCurrentAnimatorClipInfo(0).ToString() + " | " + animation.ToString());
        if (characterAnim.GetCurrentAnimatorClipInfo(0).ToString() == animation.ToString()) return;
