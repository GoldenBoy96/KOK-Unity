...
public class FusionManager : MonoBehaviour, INetworkRunnerCallbacks
{
    ...
    {
        isHost = false;
        StartCoroutine(UpdateParticipantList());
    }

    private void OnDisable()
    {
        ChatManager.Instance.SendMessageAll(runner.GetPlayerObject(runner.LocalPlayer).GetComponent<PlayerNetworkBehavior>().PlayerName + " đã rời phòng!");
        StopAllCoroutines();
    }
    IEnumerator  UpdateParticipantList() {
    ...
    public void OnPlayerLeft(NetworkRunner runner, PlayerRef player)
    {
        ...
        if (runner.LocalPlayer == runner.ActivePlayers.ToList().OrderBy(x => x.ToString()).ToList()[0]) {
            ...
            {
                runner.GetComponent<PlayerNetworkBehavior>().PlayerRole = 1;
            }
